  ...
  <div class="auth-inner py-2">
      <div class="card p-2 mb-0 bg-white">
          <div class="card-body mx-1 text-center">
              ...
              <h4 class="text-center mb-1 fw-700">¡Verifica tú Correo Electronico!</h4>
              @if (session('resent'))
              ...
              @endif
                <p class="card-text mb-3 text-centert">{{ __('Si no has recibido el correo, has click abajo para solicitar otro') }}</p>
                @if (isset($user))
                ...
                @endif
                  <input type="hidden" name="resend_email" value="1">
                    @csrf
                    <button type="submit" id="verfBTN" class="btn btn-primary fw-400 align-baseline mb-0" style="letter-spacing: 0.8px"><span class="spin spinner-border-sm"></span> {{ __('Reenviar correo') }}</button>.
                </form>
                {{-- {{ dd(request('resent')) }} --}}
...
<script>
  ...
  btnSubmit.addEventListener('click', ()=>{
    ...
    document.querySelector('.spin').classList.add('spinner-border');
  })
