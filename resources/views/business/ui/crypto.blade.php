    ...
    @if ($cryptoInves != null)
        <div class=" card">
            ...
            <div class="card-body ">
                <div class="contenedor-irv tres-irv ">
                    <article class="entrada-irv">
                        ...
                        <div class="col-sm-12 mt-3">
                            <label
                                style="color: #47586C; height: 22%; font-size: 25px; font-weight: 600">{{ $cryptoInves->ordenes->membershipPackage->amount }}
                                USDT</label>
                    ...
                    <article class="entrada-irv">
                        <div class="col-sm-12">
                            <label style="color: #47586C; height: 22%; font-size: 25px; font-weight: 600">
                                {{ $cryptoInves->start_date != null ? date('d/m/Y', strtotime($cryptoInves->start_date)) : $cryptoCreation }}
                            </label>
                        ...
                        <div class="col-sm-12 mt-3">
                            <label
                                style="color: #47586C; height: 22%; font-size: 25px; font-weight: 600">{{ $cryptoInves->ordenes->membershipPackage->amount_per_month }}
                                USDT</label>
                            <p style="color: #0255B8;">Membresía</p>
                        </div>
                    </article>

                    @if ($cryptoInves->start_date != null)
                        <article class="entrada-irv">
                            <div id="chart3" style="margin-top:-20%"></div>
                            <div class="d-flex justify-content-center" style="margin-top: -10%">
                                <p class="text-success text-bold text-center">Conectado</p>
                            </div>

                            <form id="renew-form" class="custom" action="{{ route('shop.proccess') }}" method="POST">
                                @csrf
                                <input type="hidden" name="package"
                                    value="{{ $cryptoInves->ordenes->membership_packages_id }}">
                ...
                @if ($cryptoInves->ordenes->alertNotification != null ? $cryptoInves->ordenes->alertNotification->status == '1' : false)
                    <form id="renew-form" class="custom" action="{{ route('formulary') }}" method="GET">
                        @csrf
                        <div class="d-flex justify-content-center">
                            <div class="col-sm-6">
                                <input type="hidden" name="orderId" value="{{ $cryptoInves->ordenes->id }}">
                ...
                <div class="alert alert-danger mt-1 col-sm-12" style="border-radius: 6px">
                    <div style="padding: 1rem;" class="lis">
                        ...
                        <p class="perdida">Si su irv no es renovado toda operación
                            abierta será
                            cerrada y liquidada a <br>mercado ya sea en ganancia o pérdida.
                        </p>
                    </div>
                </div>
            </div>
